...
const initial: Msg[] = [{
  role: 'ai',
  text: 'Xin chào. Tôi là HPGK AGENT v0.6.1.\n\nWeb Studio sẵn sàng cho inference on-demand. Nếu chưa có worker/checkpoint được promote, tôi sẽ báo trạng thái thay vì giả lập AI.'
}]
...
export default function App() {
  ...
  const panels: Record<string, ReactNode> = {
    ...
    'Knowledge': <><div className="card"><h3>Context & Knowledge</h3><div className="code">conversation → recent context → worker prompt → model</div></div><div className="notice">V0.6 gửi context có giới hạn. Knowledge store vẫn tách khỏi prompt để tránh biến mọi dữ liệu thành context không kiểm soát.</div></>,
    'Learning': <><div className="card"><h3>Continual learning gate</h3>{[['Candidate queue','READY'],['Checkpoint','ENABLED'],['Benchmark gate','REQUIRED'],['Unsafe auto-update','OFF'],['Training backend','KAGGLE GPU'],['Inference mode','ON-DEMAND']].map(([a,b])=><div className="row" key={a}><span>{a}</span><b className="value">{b}</b></div>)}</div><div className="code">candidate → validate → train → benchmark → promote / rollback</div></>,
    'Evaluation': <><div className="card"><h3>Coding benchmark</h3>{['HTML structure','CSS layout','JavaScript','TypeScript','React','Next.js'].map(x=><div className="row" key={x}><span>{x}</span><span className="value">READY</span></div>)}</div><div className="empty">Execution sandbox chưa bật. V0.6 chỉ gửi task và code context tới worker.</div></>,
    'Model Registry': <><div className="card"><h3>Release gate</h3>{[['Active channel','v0.6'],['Inference','REAL WORKER'],['Streaming','SSE'],['Promotion','Benchmark-gated']].map(([a,b])=><div className="row" key={a}><span>{a}</span><span className="value">{b}</span></div>)}</div><div className="notice">Model thật chỉ được dùng khi worker có checkpoint + tokenizer hợp lệ.</div></>
  }

  return <div className="app"><header className="top"><div className="brand"><div className="mark">H</div>HPGK AGENT <span>CODE INTELLIGENCE</span></div><div className="status"><i className={worker==='ready'?'live':'off'}/>{worker==='ready'?'WORKER ONLINE':'WORKER OFFLINE'} · v0.6</div></header><div className="layout"><aside className="side"><div className="label">Workspace</div>{nav.slice(0,5).map(x=><button key={x} onClick={()=>setSection(x)} className={'nav '+(section===x?'active':'')}>{x}</button>)}<div className="label">System</div><button onClick={()=>setSection('Model Registry')} className={'nav '+(section==='Model Registry'?'active':'')}>Model Registry</button></aside><main className="main"><section className="hero"><div className="eyebrow">From-zero · Vietnamese-first · Code-first</div><h1>Build. Learn. Verify.</h1><p>HPGK AGENT v0.6 nối Web Studio với inference worker thật, giữ context có giới hạn, streaming SSE và coding workflow tách biệt.</p><div className="metrics">{metrics.map(([a,b])=><div className="metric" key={a}><small>{a}</small><b>{b}</b></div>)}</div></section>{section==='Chat'?<section className="surface"><div className="head"><b>AI Chat</b><span className="status">{worker==='ready'?'LIVE INFERENCE':'NO WORKER'}</span></div><div className="body"><div className="messages">{messages.map((m,i)=><div className={'msg '+m.role} key={i}><small>{m.role==='ai'?'HPGK AGENT':'YOU'}</small>{m.text}{busy&&i===messages.length-1&&m.role==='ai'&&<span className="cursor"/>}</div>)}</div><div className="compose"><textarea value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send()}}} placeholder="Ví dụ: debug lỗi React này..."/><button className="send" onClick={send} disabled={busy}>{busy?'…':'Gửi'}</button>{busy&&<button className="tab" onClick={stop}>Stop</button>}</div></div></section>:<section className="surface"><div className="head"><b>{section}</b><span className="status">v0.6.1</span></div><div className="body">{panels[section]}</div></section>}</main><aside className="right"><div className="card"><h3>Runtime</h3>{[['Web Studio','Ready'],['Inference',worker==='ready'?'Online':'On-demand'],['Streaming','SSE'],['Context','Capped']].map(([a,b])=><div className="row" key={a}><span>{a}</span><span className="value">{b}</span></div>)}</div><div className="card"><h3>Architecture</h3><div className="code">{'Browser\n ↓\nVercel API proxy\n ↓\nHTTPS worker\n ↓\nTokenizer\n ↓\nTransformer'}</div></div><div className="card"><h3>Safety</h3><div className="notice">Worker không tự ghi file và không execute code ở V0.6. Coding agent chỉ phân tích task + context + file.</div></div></aside></div></div>
}
