...
- **`Module not found: Can't resolve '@/components/...'` và `'@/lib/db'`**
  — webpack không resolve được alias `@/` trong môi trường build thật
  của bạn, dù cấu hình `tsconfig.json` (`baseUrl` + `paths`) đúng theo
  convention chuẩn của Next.js và tôi đã xác nhận từng file đích tồn tại
  thật trên đĩa. Tôi không có cách nào tái hiện hay debug trực tiếp lỗi
  resolve-alias này trong sandbox (không chạy được `next build` ở đây).

  **Quyết định:** thay vì tiếp tục đoán nguyên nhân chính xác (Next.js
  version quirk, cách Vercel build project, hay cấu hình nào đó tôi
  không nhìn thấy được), tôi **bỏ hẳn alias `@/` và chuyển toàn bộ 18
  file trong `app/`/`components/` sang import tương đối** (`../../lib/db`
  thay vì `@/lib/db`). Import tương đối không phụ thuộc bất kỳ cấu hình
  build-tool nào (tsconfig paths, webpack alias, baseUrl) — đây là cơ
  chế nguyên thuỷ nhất của module system, không có bề mặt nào để cấu
  hình sai. Đã xoá `baseUrl`/`paths` khỏi `tsconfig.json` vì không còn
  dùng.

  **Đã xác minh toàn bộ 41 import tương đối mới** (script tự động viết
  lại, sau đó ĐỐI CHIẾU THỦ CÔNG từng cái với file thật trên đĩa bằng
  `os.path.normpath` + kiểm tra tồn tại — không tin script mù quáng),
  bao gồm cả những đường dẫn sâu 6 cấp thư mục
  (`app/api/projects/[id]/files/content/route.ts` →
  `../../../../../../lib/db`). Tất cả 41/41 đều khớp đúng.

  74/74 test lõi vẫn PASS (không bị ảnh hưởng vì `core/`/`lib/` chưa bao
  giờ dùng alias `@/`).

- **Đây là lỗi build thứ 5.** Tôi cần nói thẳng: tôi đã không thể tự xác
  nhận việc alias `@/` sẽ hoạt động đúng trong môi trường build thật của
  bạn, dù nó là cấu hình tiêu chuẩn. Việc chuyển sang import tương đối là
  lựa chọn AN TOÀN HƠN CHỨNG MINH ĐƯỢC — không phải vì tôi hiểu rõ tại
  sao alias fail, mà vì import tương đối loại bỏ hoàn toàn khả năng xảy
  ra loại lỗi này, bất kể nguyên nhân gốc là gì. `lib/zip.ts` (phụ thuộc
  `adm-zip`) vẫn là phần rủi ro lớn nhất hoàn toàn chưa build thử qua
  được ngày nào.

## Bug thật đã phát hiện SAU khi giao lần 4 (qua `next build` thật trên Vercel)

- **`lib/db.ts:80` — generic type không thể merge được với `Partial<T>`.**
  Lỗi TS: `Type '{ id?: string | undefined; } & Partial<T>' is not
  assignable to type 'T'`. Đây là một giới hạn ĐÃ BIẾT của hệ thống kiểu
  TypeScript: khi `T` là generic type parameter chưa được ràng buộc cụ
  thể (chỉ `extends { id: string }`), TypeScript không thể tự chứng minh
  rằng `{ ...mộtT, ...Partial<T> }` chắc chắn khớp lại `T` — dù về mặt
  giá trị runtime điều này LUÔN đúng (patch chỉ override field đã tồn tại
  trong T, không thể thêm field lạ).

  **Sửa:** dùng type assertion `as T` có chủ đích tại đúng điểm merge,
  kèm comment giải thích rõ vì sao an toàn. Đây là type assertion, KHÔNG
  phải `any` — không xoá bỏ type checking ở bất kỳ đâu khác, chỉ khẳng
  định một sự thật đã được chứng minh bằng logic (constraint `Partial<T>`
  đảm bảo patch là tập con hợp lệ của T). Đây là pattern được cộng đồng
  TypeScript chấp nhận rộng rãi cho đúng loại giới hạn generic-spread
  này — khác về bản chất với việc dùng `any`/`@ts-ignore` để che giấu một
  lỗi logic thật không rõ nguyên nhân.

  Nhân tiện cũng sửa luôn một lỗi `noUncheckedIndexedAccess` ẩn trong
  cùng hàm: `records[idx]` được đọc 2 lần (một lần để spread, một lần để
  return) mà không guard — giờ chỉ đọc một lần vào biến `current`, guard
  `if (idx === -1 || !current) return undefined`, và trả về biến
  `updated` đã có sẵn thay vì đọc lại mảng.

  **Đã quét toàn bộ 82 file** tìm các hàm/kiểu generic khác có thể dính
  cùng loại lỗi — xác nhận `lib/db.ts` là nơi DUY NHẤT trong toàn bộ
  source có generic type parameter (`<T>`) được định nghĩa; 2 chỗ spread
  khác trong codebase (`pipeline.ts`, `store.ts`) dùng interface CỤ THỂ
  (không phải generic chưa ràng buộc), nên không gặp giới hạn này.

  74/74 test vẫn PASS sau khi sửa.

- **Vẫn CHƯA có bằng chứng `next build` chạy hết toàn bộ và pass.** Đây
  là 4 lỗi build liên tiếp bạn đã phải tự phát hiện qua build thật — mỗi
  lỗi tôi sửa xong đều làm tăng độ tin cậy, nhưng tôi không thể khẳng
  định "sẽ không còn lỗi nào nữa" một cách trung thực khi chưa tự chạy
  được `next build` đến cuối. `lib/zip.ts` (phụ thuộc `adm-zip`, hoàn
  toàn chưa build thử) vẫn là rủi ro lớn nhất còn lại.
